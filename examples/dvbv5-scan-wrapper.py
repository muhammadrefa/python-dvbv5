...
"""
dvbv5-scan-wrapper.py

Frequency scanning tool ported from dvbv5-scan
This example utilises libdvbv5 wrapper
"""
...
def run_scan(dvb: dvb_dev.DVBDevice, dmx_dev: dvb_dev.DVBDevList, confpath: str, timeout_multiply: int = 1):
    ...
    try:
        dmx_fd = dvb_dev.dvb_dev_open(dvb, dmx_dev.sysname, os.O_RDWR)
    except:
    ...
    for entry in _dvb_file.entry:
        ...
        dvb_scan_handler = dvb_dev.dvb_dev_scan(dmx_fd, entry, check_frontend, _args, 0, timeout_multiply)
        if parms.abort:
...
def main(confpath: str, country: str = None, timeout_multiply: int = 1, verbose: int = 0):
    ...
    try:
        ...
        fe_fd = dvb_dev.dvb_dev_open(dvb, fe_dev.sysname, os.O_RDWR)
    except:
